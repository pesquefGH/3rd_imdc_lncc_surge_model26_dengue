% LNCC Surge Model 26 - Validation 4
...
% Validation 4 
ind_EW25_2025=649+52+52+52; % time index of EW 25 of 2025 
...
dcases=dcases(1:ind_EW25_2025);  % crops time-series at EW 25 of 2025 (including)
...
dcasesfs=dcasesf(41:ind_EW25_2025);  % selects filtered data from
% EW 41/2010 to EW 25/2025


% From the time-series 'dcasesfs' of the dengue surges, we obtain an 
% average surge curve.
...
P_est = lsqcurvefit(fun,P,n,typ_DC,[500;0.1;24],[370000;0.5;28],options); 
% find estimated model parameters P_est is the optimal parameter vector

P_est(3)=round(P_est(3)); % rounds estimated n0, since it is 
...
x=Model_surge;   % surge template: attributes Model_surge to x 
% (for notation clarity)  
for kk=1:ns
    a=DCalign(:,kk);   % observed surge at column kk
    g(kk)=a'*x./(x'*x);   % calculates amplitude gain for each observed 
    % surge at column kk
end
...
mg=param(1);  % mean of value of g (from 2010 to 2025)
sigma=param(2); % standard deviation of value of g (from 2010 to 2025)

MC=10000;  % number of montecarlo runs for forecast surges in 2026
...
% from EW 41 of 2025 to EW 40 of 2026
...
indf_end=716+52+52+52+1; % time index of the EW 40 2026 
...
writetable(T,['..\spreadsheets\v4_SModel26_',num2str(state_code(1)),'.csv'],'Delimiter',',')
...
figure
subplot(221)
plot(indf_ini:length(dcases_orig),cases,'linewidth',2); % plot known sequence of cases 
% from EW 41 2025 up to EW 52 of 2025
hold on;
...
print(['..\plots\v4_SModel26_',UF],'-dpdf')
